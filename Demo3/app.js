...
AFRAME.registerComponent("envio-paquetes", {
        ...
        .then(json => {
            ...
            json.packages.forEach(paquete => {
                ...
                const timeout = setTimeout(() => {
                    if (!this.pausa) {
                        numeroPaquetes++
                        this.animacion(paquete, identificador);  
                    }
                    
                    // Elimina el timeout del almacen después de ejecutarse
                    this.almacenTimeouts = this.almacenTimeouts.filter(t => t.id !== timeout);
                }, delay);
                this.almacenTimeouts.push({ id: timeout, package: paquete, delay: delay, startTime: Date.now(), remainingTime: delay, idpaquete: identificador});
            })
        })
        .catch(error => console.error('Error loading packages.json:', error));
    },

    animacion: function(paquete, id) {
    ...
    reanudar: function() {
        ...
        this.almacenTimeouts.forEach(t => {
            ...
            const timeout = setTimeout(() => {
                numeroPaquetes++
                this.animacion(t.package,t.idpaquete);
                // Elimina el timeout del almacen después de ejecutarse
                this.almacenTimeouts = this.almacenTimeouts.filter(t => t.id !== timeout);
            }, t.remainingTime);
